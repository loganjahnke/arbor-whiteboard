...
firebase.auth().onAuthStateChanged(function (user) {
    if (user) {
        ...
        database.ref('admins').orderByChild('email').equalTo(user.email).once("value", function (snapshot) {
            if (snapshot.exists()) {
                snapshot.forEach(function (child) {
                    // Get account session name
                    var accountSessionName = child.child("session").val();

                    // If admin is not allowed here, boot them.
                    if (sessionName != accountSessionName) {
                        window.location.href = "session-ended.html?error=unauthorized";
                    }
                });
            } else {
                // Check if user is a tutor
                database.ref('tutors').orderByChild('email').equalTo(user.email).once("value", function (snapshot) {
                    if (snapshot.exists()) {
                        snapshot.forEach(function (child) {
                            // Get account session name
                            var accountSessionName = child.child("session").val();
                        ...
                        database.ref('clients').orderByChild('email').equalTo(user.email).once("value", function (snapshot) {
                            if (snapshot.exists()) {
                                snapshot.forEach(function (child) {
                                    // Get account session name
                                    var accountSessionName = child.child("session").val();
